...
from sklearn.preprocessing import StandardScalerµ
...
prediction_titles = [title(y_pred[i]-1, y_test[i]-1, names)
...
plt.figure()
img = np.reshape(clf.coef_, (h, w, n_colors))
img = (img - img.min()) / (img.max() - img.min())  # mise à l’échelle [0,1]
plt.imshow(img)
plt.draw()
...
def run_svm_cv(_X, _y):
    ...
    return('Generalization score for linear kernel: %s, %s' %
          (_clf_linear.score(_X_train, _y_train), _clf_linear.score(_X_test, _y_test)))

print("Score sans variable de nuisance")
print(run_svm_cv(X,y))
...
ind=np.argmax(scores)
print('Nombre de composante retenu :',n_components(ind))
